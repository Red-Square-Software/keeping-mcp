...
- [x] **WRITE-06**: Write tools accept a `purpose` field matching Keeping's real OpenAPI enum: `work`, `break`, `special_leave`, `unpaid_leave`, `statutory_leave`, `sick_leave`, `work_reduction`, `trip` (default `work`). Billable status is set at the PROJECT level in Keeping, NOT on the entry, so Jortt invoicing keys off project configuration regardless of purpose.
...
- [x] **TIMER-02**: When shipped, timer tools use `X-Server-Time-Ms` from response headers to compute elapsed time correctly
