...
import { registerTasks } from "../../src/tools/tasks.js";

async function buildClient(mockClient: Partial<KeepingClient>) {
  const server = new McpServer({ name: "test", version: "0.0.0" });
  registerTasks(server, mockClient as KeepingClient);
...
describe("keeping_tasks tool", () => {
  ...
  it("Test 2: feature disabled — 404 returns graceful note, NOT isError (META-02)", async () => {
    ...
    expect(content[0]?.text).toBe("Tasks feature not enabled for this organisation.");
  });
  ...
  it("Test 5: tools/list reports readOnlyHint: true on keeping_tasks (READ-03)", async () => {
    ...
    const tool = list.tools.find((t) => t.name === "keeping_tasks");
